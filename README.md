...
-   `results`: A JSON string representing an array of objects, where each object corresponds to a step executed by the action. Each object contains the following fields:
    -   `id`: The unique identifier assigned to the step during execution (e.g., `action_0_actions-checkout`, `action_1_run`).
    -   `name`: The human-readable name of the step, taken from the original action definition in the `presets` or `action_list` input. If a name wasn't provided, it defaults to a generated name (e.g., `Run actions/setup-node@v4` or `Run script 1`). For steps generated by `act` itself (like setup/cleanup), the `id` might be used as the name.
    -   `outcome`: The result of the step execution (e.g., `success`, `failure`, `skipped`).
    -   `outputs`: An object containing the outputs set by the step.
    ...
    **Example `results` Output:**
    ...
    [
      {
        "id": "action_0_actions-checkout",
        "name": "Checkout code",
        "outcome": "success",
        "outputs": {}
      },
      {
        "id": "action_1_actions-setup-node",
        "name": "Setup Node.js LTS",
        "outcome": "success",
        "outputs": {
          "node-version": "20.11.1",
          "cache-hit": "false",
          "node-path": "/opt/hostedtoolcache/node/20.11.1/x64"
        }
      },
      {
        "id": "action_2_run",
        "name": "Install NPM Dependencies (from preset)",
        "outcome": "success",
        "outputs": {}
      },
      {
