...
-   `results`: A JSON string containing the results (including `outputs` and `outcome`) of each executed step, as captured by the final step in the dynamically generated workflow run by `act`. The structure matches the GitHub Actions `steps` context object.
    **Note:** The action also automatically prints this JSON output (pretty-printed) to the workflow logs within a collapsible group named "Super-Action Collected Results (JSON)".
