...
-   `results`: A JSON string representing an array of objects, where each object corresponds to a step executed by the action (excluding the internal `collect_results_step`). Each object contains the following fields:
    -   `name`: The human-readable name of the step, taken from the original action definition in the `presets` or `action_list` input. If a name wasn't provided, it defaults to a generated name (e.g., `Run actions/setup-node@v4` or `Run script 1`). For steps generated by `act` itself (like setup/cleanup), the step ID might be used as the name.
    -   `uses`: The action string specified (e.g., `actions/setup-python@v5`), or `null` if the step used `run`.
    -   `run`: The command string specified, or `null` if the step used `uses`.
    -   `outcome`: The result of the step execution (e.g., `success`, `failure`, `skipped`).
    -   `outputs`: An object containing the outputs explicitly set by the step using the `::set-output::` command. Standard output/error are not captured here.
    **Note:** The action also automatically prints this JSON output (pretty-printed) to the workflow logs within a collapsible group named "Super-Action Collected Results (JSON)", unless disabled by the `display_results: 'false'` input.
